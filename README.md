# BLACKJACK
## REGLAS
- Hay una casa por cada jugador
- Los valores de las cartas de los jugadores se suman.
- El objetivo de los jugadores es sumar 21 puntos.
- Los jugadores pueden dejar de jugar antes de llegar a 21 puntos sin perder.
- Si un jugador se pasa de 21 puntos, pierde.
- Si un jugador tiene exactamente 21 puntos, se dice "BLACKJACK" y gana automaticamente a la casa.
- Después de que el jugador termine de pedir sus cartas, recien entonces la casa empieza a pedir
    - Primero revela la que estaba oculta.
    - Después sí o sí pide una carta (teniendo 3 en total).
    - La casa puede pedir cuantas cartas quiera.
        - Si la casa se pasa de 21 puntos, pierde.
        - Entre la casa y el jugador: el que tenga más puntos sin pasarse de 21 gana.

## PASO A PASO
- [X] El programa se inicializa (se olvidan las cartas que los jugadores y las casas tengan en sus mazos, así como la cantidad).
- [X] El programa solicita la cantidad de jugadores. Habrán igual cantidad de casas como de jugadores.
- [X] La casa saca dos cartas por jugador: muestra una y oculta la otra
- [ ] A cada jugador se le dan dos cartas y todos las pueden ver
- [ ] En cada turno del jugador, el jugador pide una carta (alternativamente puede pedir la cantidad que quiera)
- [ ] El jugador puede dejar de pedir cartas en cualquier momento, habilitando a la casa a pedir cartas y terminar el juego.
